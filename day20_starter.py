...
channels = {
    ...
    "cpu_pct":    SensorChannel("CPU %",         "%",         "yellow"),
    "mem_pct":    SensorChannel("Memory %",      "%",         "magenta"),
    "disk_read":  SensorChannel("Disk Read",     "KB/s",      "blue") 
}

def system_reader():
   
    last_disk_bin = psutil.disk_io_counters().read_bytes
    ...
    while running:
        ...
        current_disk_bin = psutil.disk_io_counters().read_bytes
        delta_kb = (current_disk_bin - last_disk_bin) / 1024.0  
        channels["disk_read"].push(delta_kb)
        last_disk_bin = current_disk_bin
